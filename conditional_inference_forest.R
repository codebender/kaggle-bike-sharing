...
set.seed(415)
casual_fit <- cforest(as.factor(casual) ~ hour + month + year + weekday + weather + atemp +
               workingday + holiday + windspeed + humidity + season,
               data = train, controls=cforest_unbiased(ntree=100, mtry=3))
registered_fit <- cforest(as.factor(registered) ~ hour + month + year + weekday + weather + atemp +
               workingday + holiday + windspeed + humidity + season,
               data = train, controls=cforest_unbiased(ntree=100, mtry=3))
